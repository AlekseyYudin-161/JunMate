...
TASK 3a - кнопки feedback/Сброс диалога
```
...
TASK 3b - кнопка «Новое резюме» с возвратом на главную
```
Цель: после показа резюме дать пользователю выход на стартовый экран (UX-пробел —
раньше вернуться можно было только через «Доделать» → «Сбросить диалог»).

Реализовано в screens/chat.py (agents/*, core/*, welcome, логика диалога/рендера/PDF не затронуты):

1. Третья кнопка «🆕 Новое резюме» (type="secondary") в блоке предпросмотра резюме —
   отдельной строкой ниже «Скачать PDF» / «Доделать», по центру через st.columns([1, 2, 1]),
   с отступом в две пустые строки (st.write("") дважды) для защиты от случайного нажатия.

2. Подтверждение через модалку @st.dialog("Вернуться на главную"):
   текст «Возвращаемся на стартовую страницу, ваш диалог сбросится.»
   - «Отмена» (type="secondary") — закрывает модалку, ничего не сбрасывает;
   - «Да, возвращаемся» (type="primary") — очищает весь session_state и st.rerun()
     (init_state восстанавливает screen="welcome" → чистый стартовый экран).

ВАЖНО (решённый баг): очистку session_state нельзя делать инлайн в теле render_chat_screen —
после удаления ключей код продолжает выполняться и обращается к удалённым ключам → пустой
экран. Решение: очистка выполняется ВНУТРИ @st.dialog (изолированный контекст, как у
reset_dialog), а не в теле функции. Кнопка лишь вызывает модалку.
Цвета — штатные type=primary/secondary (Streamlit адаптирует под тему), без CSS.
Сделай, я сам проверю.
```
