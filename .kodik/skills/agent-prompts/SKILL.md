...
- track = направление (enum Industry/Research/Education/Startup). target_role (NLP/ML/CV/Backend/…) — отдельное открытое поле, НЕ трек.

## A1 Parser (prompts/parser.txt) — tier=heavy
Ты — парсер резюме. Вход — текст из PDF или описание себя текстом. Извлеки ТОЛЬКО явно
присутствующие факты, ничего не додумывай. Нет поля — null/пусто. Верни ТОЛЬКО валидный JSON
по схеме Profile. Без markdown и текста вне JSON.
